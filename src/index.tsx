...
import { CropDialog, type CropDialogProps } from './CropDialog';
import { Uploader } from './Uploader';
import type { UploaderProps } from './Uploader';
import { ImageCard } from './ImageCard';
import type { ImageCardProps } from './ImageCard';
import { PreviewDialog } from './PreviewDialog';
import { ToolbarRender as defaultToolbarRender } from './CropDialog/ToolbarRender';
import { ActionsRender as defaultActionsRender } from './CropDialog/ActionsRender';
import { TitleRender as defaultTitleRender } from './CropDialog/TitleRender';
...
export const ImageCrop = (props: React.PropsWithChildren<ImageCropProps>) => {
  const {
    aspectMarks = defaultAspectMarks,
    accept = defaultAccept,
    uploaderProps,
    children,
    disabled,
    filename,
    imageCardProps: imageCardPropsProp,
    error,
    preview = true,
    previewDialogProps,
    locale = 'en',
    ...
    imageCropDialogContentRootStyle,
    imageType,
    onFinish: onFinishProp,
    defaultAspect: defaultAspectProp,
    okText,
    resetText,
    cancelText,
    originText,
    TitleRender = defaultTitleRender,
    ActionsRender = defaultActionsRender,
    ToolbarRender = defaultToolbarRender,
    ...restProps
  } = props;
  const imageCardProps = {
    ...(locale === 'zh-cn' ? imageCardDefaultCNProps : imageCardDefaultENProps),
    ...(imageCardPropsProp || {}),
  };
  const [imageInfo, setImageInfo] = useSafeState<CropDialogProps['imageInfo'] | null>(null);
        ...
        <PreviewDialog
          ...
          {...{
            ...
            onClose: () => {
              ...
              (previewDialogProps?.onClose as any)?.();
            },
          }}
        />
      )}
...
export interface ImageCropProps extends Partial<CropperProps> {
  ...
  */
  ...
  TitleRender?: typeof defaultTitleRender | React.ReactNode;
  /** 显示调整长高比工具条?
   * @default true
   */
   ...
   * ```
   */
  ToolbarRender?: typeof defaultToolbarRender | React.FC | React.Component;
  /** 自定义底部按钮区域的组件
   * ```
   * //以下props会被传递
   ...
   * ```
   */
  ActionsRender?: typeof defaultActionsRender | React.FC | React.Component;
  /** 重置按钮的(文本)内容
   * @default 'Reset'
   */
